...
class ConfigManager:
    ...
    def test(self):
        url = f'{self.host}/api/ha/scenes'
        response = urequests.get(url)
        data = response.json()
        #token = data.get('token')
        print(data)
        #target = { "entity_id": "scene.test"}
        print(data.get("scenes"))
        #req = urequests.post(f"http://homeassistant.local:8123/api/services/scene/turn_on", headers={"Authorization": f"Bearer {token}"}, json=target)
        #data = req.json()
        #print(data)
